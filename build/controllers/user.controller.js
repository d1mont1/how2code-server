...
exports.registrationUser = (0, catchAsyncError_1.catchAsyncError)(async (req, res, next) => {
    try {
        ...
        try {
            ...
            res.status(201).json({
                success: true,
                message: `Пожалуйста, проверьте свою почту ${user.email} чтобы активировать ваш аккаунт.`,
...
exports.updateAccessToken = (0, catchAsyncError_1.catchAsyncError)(async (req, res, next) => {
    try {
        ...
        await redis_1.redis.set(user._id, JSON.stringify(user), 'EX', 86400); // Количество секунд в 1 дне
        return next();
    }
    catch (error) {
        return next(new ErrorHandler_1.default(error.message, 400));
...
//Получить всех пользователей --- только для админa
...
//Обновление роли пользователя --- только для админa
...
//Удаление пользователя --- только для админa
